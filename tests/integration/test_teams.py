...
async def test_teams_with_no_auth(client: "AsyncClient") -> None:
    ...
    Inertia uses 307 for GET (preserves method) and 303 for other methods (redirects to GET).
    """
    # PATCH/POST/DELETE return 303 (See Other)
    ...
    assert response.status_code == 303
    # GET returns 307 (Temporary Redirect)
...
async def test_teams_list(client: "AsyncClient", superuser_inertia_headers: dict[str, str]) -> None:
    ...
    assert response.status_code == 200
    data = response.json()
    # Inertia responses wrap data in props, with pagination data in "content"
    assert "props" in data
    assert "content" in data["props"]
    assert "items" in data["props"]["content"]
    assert "total" in data["props"]["content"]
    assert int(data["props"]["content"]["total"]) > 0
...
async def test_teams_get(client: "AsyncClient", superuser_inertia_headers: dict[str, str]) -> None:
    ...
    assert response.status_code == 200
    data = response.json()
    # Inertia responses wrap data in props with content containing the team
    assert "props" in data
    assert "content" in data["props"]
    assert data["props"]["content"]["name"] == "Test Team"
...
async def test_teams_delete(client: "AsyncClient", superuser_inertia_headers: dict[str, str], superuser_token_headers: dict[str, str]) -> None:
